...
class SimplifiedCRMConsumer {
   ...
   */
  async checkUserPermission(userId, permission, userPermissions = null) {
    try {
      // If permissions provided directly, use them
      if (userPermissions) {
        return userPermissions.includes(permission) || userPermissions.includes('*');
      }

      // Fallback: get permissions from user context
      const userContext = await this.getUserOrganizationContext(userId);
      if (!userContext?.permissions) return false;

      return userContext.permissions.includes(permission) || userContext.permissions.includes('*');
    } catch (error) {
  ...
  async logActivity(activityData) {
    ...
    try {
      ...
      const activity = {
        ...
        action: activityData.operation, // Required field: map operation to action
        entityType: activityData.entityType,
        entityId: activityData.entityId,
        details: { // Required field: structured details object
